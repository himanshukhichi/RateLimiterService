...
public class FixedWindowCounterRateLimiter implements RateLimiter {
    ...
    public FixedWindowCounterRateLimiter(StringRedisTemplate redisTemplate, RateLimiterKeyFactory keyFactory) {
        this.redisTemplate = redisTemplate;
        this.keyFactory = keyFactory;
        this.script = new DefaultRedisScript<>();
        this.script.setScriptSource(new ResourceScriptSource(new ClassPathResource("scripts/fixed_window_counter.lua")));
    ...
    public RateLimitDecision check(RateLimitRequest request) {
        ...
        if (result == null || result.size() < 4) {
            throw new IllegalStateException("Redis fixed window script returned no decision");
        }
